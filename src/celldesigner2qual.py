...
def species_info(model):
    ...
    # TODO: Pure SBML doesn't have complexspecies info
    nameconv = {}
    compartments = {}
...
def write_qual(
    filename: str, info, width: str, height: str, remove: int = 0, sif: bool = False, qual: bool = False
):
    ...
    for species, data in info.items():
        if data["transitions"]:
            ...
            if len(ilist) == 0:
                ...
                add_function_as_rdf(info, species, posfunc + " " + negfunc)  # FIXME
            else:
                olist = etree.SubElement(trans, "qual:listOfOutputs")
                etree.SubElement(
                    olist,
                ...
                info[species]["function"] = (posfunc, negfunc)
                add_function_as_rdf(info, species, posfunc + " " + negfunc) # FIXME: rdf?
                add_notes(trans, data["transitions"])
                add_annotations(trans, data["transitions"])
...
def map_to_model(map_filename: str, model_filename: str, bma=False):
    ...
    logger.disable("")
    print("parsing: {0}".format(map_filename))
