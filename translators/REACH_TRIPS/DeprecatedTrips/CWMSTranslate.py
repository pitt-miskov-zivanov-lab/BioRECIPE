...
class TripsTranslator(object):
	...
	def xml_to_dict(self):
		...
		for element in self.root:
			...
			if (element.tag == 'TERM'):
				...
				if (element.attrib['rule'] == '-SIMPLE-REF' or 'NORM' in element.attrib['rule']):
					...
					term_name = element.find('text').text
					...
					term_norm = element.find('text').get('normalization')
					attrib_dict['term_norm'] = term_norm
			...
			# records EVENT type attributes		
				...
				event_name = element.find('text').text
				attrib_dict['event_name'] = event_name
				event_norm = element.find('text').get('normalization')
				attrib_dict['event_norm'] = event_norm
	...
	def refine_dict(self):
		...
		# tries to reassign causal connective elements
		for cc_id, attrib_dict in self.cc_dict.items():
			try:
				...
				logger.debug('CC is disconnected \n')
				pass
